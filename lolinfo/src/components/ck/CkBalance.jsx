...
import { Controls, Handle, Position, ReactFlow, ReactFlowProvider, useReactFlow } from "@xyflow/react";
...
function MatchGroups({ rows, onSelect, selectedMatch }) {
    return <div className="balance-match-groups">
        {POSITIONS.map(position => {
            const positionRows = rows.filter(row => row.position === position);
            if (!positionRows.length) return null;
            return <section className="balance-match-group" key={position} aria-labelledby={`balance-list-group-${position}`}>
                <h4 id={`balance-list-group-${position}`} style={{ "--balance-position-color": POSITION_COLORS[position] }}>{position} <span>{LABELS[position]}</span><em>{positionRows.length}명</em></h4>
                {positionRows.map(row => <MatchRow key={balanceNodeId(row)} row={row}
                    active={selectedMatch?.opponentNo === row.opponentNo && selectedMatch?.position === row.position}
...
const BalanceLaneNode = memo(function BalanceLaneNode({ data }) {
    ...
    return <section className={`balance-lane-node ${active ? "" : "is-inactive"}`} style={{ width: nodeWidth, "--balance-position-color": POSITION_COLORS[position] }} role="group" aria-label={`${LABELS[position]} 맞라인 상대`} aria-disabled={!active}>
        <div className="balance-lane-heading">
...
function BalanceFlow({ center, rows, loading, onSelect, onShowMore, positionFilter }) {
    ...
    const [size, setSize] = useState({ width: 900, height: 740 });
    useEffect(() => {
    ...
    const nodes = useMemo(() => {
        ...
        const rootWidth = isMobileLayout ? Math.min(120, width - 28) : 180;
        const laneWidth = isMobileLayout ? Math.min(120, (width - 12) / 3) : Math.min(235, Math.max(155, width * 0.245));
        const sideX = { left: isMobileLayout ? 4 : 12, right: width - laneWidth - (isMobileLayout ? 4 : 12), center: centerX - laneWidth / 2 };
        const items = [{ id: "balance-center", type: "balanceStreamer", position: { x: centerX - rootWidth / 2, y: centerY - (isMobileLayout ? 68 : 76) }, data: { streamer: center, root: true, loading, onSelect, nodeWidth: rootWidth } }];
        const laneLayout = {
            TOP: { x: sideX.center, y: 8 },
            JUG: { x: sideX.right, y: isMobileLayout ? 30 : 54 },
            MID: { x: sideX.right, y: height - (isMobileLayout ? 270 : 282) },
            AD: { x: sideX.center, y: height - (isMobileLayout ? 250 : 260) },
            SUP: { x: sideX.left, y: height - (isMobileLayout ? 270 : 282) },
        };
        POSITIONS.forEach(position => items.push({
        ...
        return items;
    }, [center, isMobileLayout, loading, onSelect, onShowMore, positionFilter, rowsByPosition, size]);
    ...
    return <div className="balance-flow" ref={flowRef} aria-label="맞라인 상대 네트워크 그래프">
        <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} fitView fitViewOptions={{ padding: 0.12, minZoom: 0.35, maxZoom: 1 }} minZoom={0.35} maxZoom={2}
            ...
            <Controls showInteractive={false} position="bottom-left" aria-label="그래프 확대·축소 및 전체 보기" />
        </ReactFlow>
...
function BalanceResults({ baseNo, knownName, knownSoopId }) {
    ...
    const [basePositionFilter, setBasePositionFilter] = useState("ALL");
    ...
    const load = useCallback(async (target, position) => {
        ...
        const isRoot = streamerNo === Number(baseNo) && !position;
        ...
        if (cacheRef.current.has(key)) {
            if (isRoot) setBaseRows(cacheRef.current.get(key));
    ...
    const onSelect = useCallback(row => {
        ...
        setSelectedMatch({ ...row, sourceName: displayCenter.streamerName });
        setPositionFilter(row.position || "ALL");
        load({ streamerNo: row.opponentNo, streamerName: row.opponentName, streamerSoopId: row.opponentSoopId }, row.position);
    }, [displayCenter.streamerName, load, status]);
    ...
    const baseListRows = useMemo(() => POSITIONS.flatMap(position => baseRows.filter(row => row.position === position && (basePositionFilter === "ALL" || basePositionFilter === position))), [basePositionFilter, baseRows]);
    const listRows = useMemo(() => status === "ready" ? POSITIONS.flatMap(position => filteredRows.filter(row => row.position === position)) : [], [filteredRows, status]);
    const showingRoot = !selectedMatch;
    ...
    return <>
        <section className="balance-panel balance-results-panel" aria-labelledby="balance-opponents">
            ...
            {status === "error" && view === "graph" && <div className="balance-error" role="alert"><span>맞라인 기록을 불러오지 못했습니다.</span><button type="button" className="btn btn-outline-light btn-sm" onClick={retry}>다시 시도</button></div>}
            {view === "graph" && status === "ready" && !filteredRows.length && <p className="balance-empty">{rows.length ? "선택한 포지션에서 확인된 맞라인 상대가 없습니다." : "등록된 맞라인 기록이 없습니다."}</p>}
            ...
            {view === "list" && <div className="balance-list-results">
                <section className="balance-list-column" aria-labelledby="balance-base-list-title">
                    <div className="balance-list-heading-row">
                        <h3 id="balance-base-list-title" className="balance-list-heading">{root.streamerName}의 맞라인 상대 <span>{baseListRows.length}명</span></h3>
                        <div className="balance-list-filter" aria-label="기준 스트리머 목록 포지션 필터">
                            <span>기준 목록</span>
                            <select value={basePositionFilter} onChange={event => setBasePositionFilter(event.target.value)} aria-label="기준 스트리머 목록 포지션">
                                <option value="ALL">전체</option>
                                {POSITIONS.map(position => <option key={position} value={position}>{LABELS[position]}</option>)}
                            </select>
                        </div>
                    </div>
                    {status === "error" && showingRoot && <div className="balance-list-state balance-error" role="alert"><span>기준 목록을 불러오지 못했습니다.</span><button type="button" className="btn btn-outline-light btn-sm" onClick={retry}>다시 시도</button></div>}
                    {!baseListRows.length && status !== "error" && <p className="balance-muted">{status === "loading" && showingRoot ? "기준 목록을 불러오는 중입니다." : "해당 포지션의 맞라인 기록이 없습니다."}</p>}
                    <div className="balance-list-scroll"><MatchGroups rows={baseListRows} onSelect={onSelect} selectedMatch={selectedMatch} /></div>
                </section>
                <section className="balance-list-column" aria-labelledby="balance-next-list-title">
                    <div className="balance-list-heading-row">
                        <h3 id="balance-next-list-title" className="balance-list-heading">{selectedMatch ? `${selectedMatch.opponentName}의 맞라인 상대` : "새 맞라인 상대"} <span>{selectedMatch ? listRows.length : 0}명</span></h3>
                        {selectedMatch && <div className="balance-list-filter" aria-label="선택 상대 목록 포지션 필터">
                            <span>선택 상대 목록</span>
                            <select value={positionFilter} onChange={event => setPositionFilter(event.target.value)} aria-label="선택 상대 목록 포지션">
                                <option value="ALL">전체</option>
                                {POSITIONS.map(position => <option key={position} value={position}>{LABELS[position]}</option>)}
                            </select>
                        </div>}
                    ...
                    {selectedMatch ? <>
                        ...
                        {status === "error" && <div className="balance-list-state balance-error" role="alert"><span>선택 상대의 목록을 불러오지 못했습니다.</span><button type="button" className="btn btn-outline-light btn-sm" onClick={retry}>다시 시도</button></div>}
                        {status === "ready" && !listRows.length && <p className="balance-muted">맞라인 상대가 없습니다.</p>}
                        <div className="balance-list-scroll"><MatchGroups rows={listRows} onSelect={onSelect} /></div>
                    </> : <p className="balance-muted">상대를 선택하면 여기에 새 목록이 표시됩니다.</p>}
                </section>
            </div>}
        </section>
    </>;
    ...
    return <div className="balance-page">
        ...
        <div className="balance-grid">
            <section className="balance-panel balance-base-panel" aria-labelledby="balance-base">
                ...
                <div className="balance-base-columns">
                    <div className="balance-base-search">
                    ...
                    <div className="balance-base-selected">
                        <span className="form-label balance-base-column-label">선택한 스트리머</span>
                        {baseNo ? <SelectedBase key={`${baseNo}-${knownName}`} baseNo={baseNo} knownName={knownName} knownSoopId={knownSoopId} onResolved={handleBaseResolved} /> : <p className="balance-muted">검색 결과에서 기준 스트리머를 선택하세요.</p>}
                    </div>
                </div>
            </section>
...
function SelectedBase({ baseNo, knownName, knownSoopId, onResolved }) {
    ...
    return <div className="balance-base-selection"><span className="balance-person"><ProfileImage soopId={soopId} /><strong>{name || "이름 확인 중…"}</strong></span></div>;
}
